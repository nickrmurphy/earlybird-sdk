...
describe("Path Utilities", () => {
	describe("normalizePath", () => {
		...
		test("converts backslashes to forward slashes", () => {
			...
			expect(normalizePath("data\\user\\profile.json")).toBe(
		...
		test("handles complex mixed cases", () => {
			expect(normalizePath("/data\\\\user//file.txt/")).toBe(
				"data/user/file.txt",
			);
			expect(normalizePath("\\\\data///user\\file.txt//")).toBe(
				"data/user/file.txt",
	...
	describe("joinPath", () => {
		test("joins simple path segments", () => {
			...
			expect(joinPath("data", "user", "profile.json")).toBe(
				"data/user/profile.json",
			);
		});
		...
		test("normalizes result", () => {
			expect(joinPath("data\\", "\\user", "file.txt")).toBe(
				"data/user/file.txt",
			);
			expect(joinPath("/data//", "//user//", "file.txt")).toBe(
				"data/user/file.txt",
		...
		test("handles complex cases", () => {
			expect(joinPath("data/", "/user/", "/nested/", "file.txt")).toBe(
				"data/user/nested/file.txt",
			);
			expect(joinPath("", "", "data", "", "file.txt", "")).toBe(
				"data/file.txt",
			);
		});
	});
