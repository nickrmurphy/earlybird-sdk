...
describe('CapacitorStorageAdapter', () => {
    ...
    describe('read operation', () => {
        ...
        test('handles null content by throwing StorageError', async () => {
            ...
            await expect(adapter.read('null.txt')).rejects.toThrow('Invalid contents at path: null.txt');
        });
        ...
        test('throws StorageError for invalid content type', async () => {
            mockFilesystem.readFile.mockResolvedValue({ data: 123 as any }); // Invalid type

            await expect(adapter.read('invalid.txt')).rejects.toThrow(StorageError);
            await expect(adapter.read('invalid.txt')).rejects.toThrow('Invalid contents at path: invalid.txt');
        });

        test('throws error for invalid path', async () => {
            await expect(adapter.read('../../../etc/passwd')).rejects.toThrow('Invalid path: ../../../etc/passwd');
    ...
    describe('write operation', () => {
        ...
        test('throws error for invalid path', async () => {
            await expect(adapter.write('../../../etc/passwd', 'evil')).rejects.toThrow('Invalid path: ../../../etc/passwd');
    ...
    describe('delete operation', () => {
        ...
        test('throws StorageError when delete fails', async () => {
            ...
            await expect(adapter.delete('nonexistent.txt')).rejects.toThrow(StorageError);
            await expect(adapter.delete('nonexistent.txt')).rejects.toThrow('Failed to delete file at path: nonexistent.txt');
        });

        test('throws error for invalid path', async () => {
            await expect(adapter.delete('../../../etc/passwd')).rejects.toThrow('Invalid path: ../../../etc/passwd');
    ...
    describe('exists operation', () => {
        ...
        test('throws error for invalid path', async () => {
            await expect(adapter.exists('../../../etc/passwd')).rejects.toThrow('Invalid path: ../../../etc/passwd');
    ...
    describe('list operation', () => {
        ...
        test('throws StorageError when listing fails', async () => {
            ...
            await expect(adapter.list('nonexistent')).rejects.toThrow('Failed to list files in directory: nonexistent');
        });

        test('throws error for invalid directory path', async () => {
            await expect(adapter.list('../../../etc')).rejects.toThrow('Invalid path: ../../../etc');
    ...
    describe('error handling', () => {
        ...
        test('Capacitor errors are wrapped in StorageError', async () => {
            ...
            try {
                ...
                expect((error as StorageError).message).toContain('Failed to delete file at path: test.txt');
            }
        });
        ...
        test('Blob conversion errors are propagated appropriately', async () => {
            ...
            failingBlob.text = mock(() => Promise.reject(new Error('Blob conversion failed')));

            mockFilesystem.readFile.mockResolvedValue({ data: failingBlob });

            await expect(adapter.read('blob.txt')).rejects.toThrow('Blob conversion failed');
        });
    });
});
