...
import { InMemoryStorageAdapter } from '../../../src/storage/adapters/InMemoryStorageAdapter.js';
...
describe('InMemoryStorageAdapter', () => {
  ...
  describe('Core StorageAdapter Interface', () => {
    ...
    describe('delete operation', () => {
      ...
      test('throws error when deleting non-existent file', async () => {
        ...
        await expect(adapter.delete('nonexistent.txt')).rejects.toThrow('File not found');
      });
  ...
  describe('Path Handling', () => {
    ...
    test('rejects invalid paths with path traversal', async () => {
      await expect(adapter.write('../file.txt', 'content')).rejects.toThrow(StorageError);
      await expect(adapter.read('../../etc/passwd')).rejects.toThrow(StorageError);
      await expect(adapter.delete('data/../file.txt')).rejects.toThrow(StorageError);
      await expect(adapter.exists('data\\\\..\\\\file.txt')).rejects.toThrow(StorageError);
      await expect(adapter.list('../')).rejects.toThrow(StorageError);
    });

    test('rejects paths with null bytes', async () => {
      await expect(adapter.write('file\x00.txt', 'content')).rejects.toThrow(StorageError);
      await expect(adapter.read('data/file\x00name.txt')).rejects.toThrow(StorageError);
    });
